...
namespace VWSDK
{
	...
	private:
		...
		RESULT					Open(IN int _nDevWidth, IN int _nDevHeight, IN int _pixelSize);
		RESULT					Close();

	public:
		VwCXPStream(DS_HANDLE _hDs, IN int _nDevWidth, IN int _nDevHeight, IN int _pixelSize, size_t nBufferCount = 2 );
		virtual ~VwCXPStream();
