...
#define CHECK_GENTL(message, function) \
	...
	<< std::string("Error calling ") << s << std::string("()") << std::endl\
	<< std::string(pc);\
...
#define THROW_GENTL(message) throw CGenTLException((message), __FILE__, __FUNCTION__, __LINE__)
	...
	private:
		...
		void makeMessage()
		{
			...
			Message << "\t" << m_FunctionName << "\t" << ShortFileName << "\t" << m_SourceLine;
...
#define _VWCATCH_ \
	...
	{\
		TCHAR tchWhat[4096]		= _T("");\
		size_t nWhatSize, nConvertWhatSize;\
		nWhatSize		= strlen(e.what())+1;\
		mbstowcs_s(&nConvertWhatSize, tchWhat, nWhatSize, e.what(), _TRUNCATE);\
		LPCWSTR strReport = (LPCWSTR)tchWhat;\
		ReportEvent( _hEventVwCXPLog, EVENTLOG_ERROR_TYPE, NULL, MSG_ERROR_RETURN_VALUE, NULL, 1, 0, &strReport, NULL );\
	} \
	catch (CGenTLException &e) \
	{\
		TCHAR tchWhat[4096]		= _T("");\
		size_t nWhatSize, nConvertWhatSize;\
		nWhatSize		= strlen(e.what())+1;\
	...
	{\
		TCHAR tchWhat[4096]		= _T("Unknown Exception");\
		LPCWSTR strReport = (LPCWSTR)tchWhat;\
		ReportEvent( _hEventVwCXPLog, EVENTLOG_ERROR_TYPE, NULL, MSG_ERROR_RETURN_VALUE, NULL, 1, 0, &strReport, NULL );\
	}
