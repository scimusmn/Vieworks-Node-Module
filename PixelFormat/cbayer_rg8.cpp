...
namespace PixelFormat
{
    namespace Bayer
    {
        CBayer_RG8::CBayer_RG8(uint32_t unWidth, uint32_t unHeight, uint32_t unBpp)
            : IBayer(unWidth, unHeight, unBpp)
        {
            m_pCovertedImage = new uint8_t[m_unWidth * m_unHeight * BGR_PIXEL];
        }

        CBayer_RG8::~CBayer_RG8()
        {
            DEBUG_LOG("CBayer_RG8::~CBayer_RG8()")
            SAFE_DELETE_ARRAY(m_pCovertedImage)
        }

        uint8_t* CBayer_RG8::ToBGR8(uint8_t *pRawImage)
        {
            ConvertBAYRG8ToBGR8(pRawImage, m_pCovertedImage, m_unWidth, m_unHeight);

            return m_pCovertedImage;
        }

        uint8_t *CBayer_RG8::ToRGB8(uint8_t *pRawImage)
        {
            ConvertBAYRG8ToRGB8(pRawImage, m_pCovertedImage, m_unWidth, m_unHeight);

            return m_pCovertedImage;
        }

        bool CBayer_RG8::SaveImage()
        {
            DEBUG_LOG("Does not Implement.")
        }

        void CBayer_RG8::ConvertBAYRG8ToBGR8(const BYTE* pbSrc, BYTE* pbDst, DWORD width, DWORD height)
        {
        ...
        */
        ...
        void CBayer_RG8::ConvertBAYRG8ToRGB8(const BYTE* pbSrc, BYTE* pbDst, DWORD width, DWORD height)
        {
            //BYTE* pbtempDst = pbDst + (height - 1) * width * 3;
