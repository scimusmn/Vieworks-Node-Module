...
namespace PixelFormat
{
    namespace Bayer
    {
        ...
        CBayer_GR10::CBayer_GR10(uint32_t unWidth, uint32_t unHeight, uint32_t unBpp)
            : IBayer(unWidth, unHeight, unBpp)
        {
            m_pCovertedImage = new uint8_t[m_unWidth * m_unHeight * BGR_PIXEL];
        }

        CBayer_GR10::~CBayer_GR10()
        {
            DEBUG_LOG("CBayer_GR10::~CBayer_GR10()")
            SAFE_DELETE_ARRAY(m_pCovertedImage)
        }


        uint8_t* CBayer_GR10::ToBGR8(uint8_t *pRawImage)
        {
            ConvertBAYGR10ToBGR8((WORD*)pRawImage, m_pCovertedImage, m_unWidth, m_unHeight);

            return m_pCovertedImage;
        }

        uint8_t* CBayer_GR10::ToRGB8(uint8_t *pRawImage)
        {
            ConvertBAYGR10ToRGB8((WORD*)pRawImage, m_pCovertedImage, m_unWidth, m_unHeight);

            return m_pCovertedImage;
        }

        bool CBayer_GR10::SaveImage()
        {
            DEBUG_LOG("Does not Implement.")
        }


        void CBayer_GR10::ConvertBAYGR10ToBGR8(const WORD* pbSrc, BYTE* pbDst, DWORD width, DWORD height)
        {
        ...
        void CBayer_GR10::ConvertBAYGR10ToRGB8(const WORD* pbSrc, BYTE* pbDst, DWORD width, DWORD height)
        {
            //BYTE* pbDst = pbDst + (height - 1) * width * 3;
            *pbDst++ = RIGHT;
            *pbDst++ = SELF;
